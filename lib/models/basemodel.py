...
class BaseModel():
    ...
    ##
    def save_weights(self, epoch:int, is_best:bool=False,type = 0):
        ...
        if is_best and type == 0:
            torch.save({'epoch': epoch, 'state_dict': self.netg.state_dict()}, f'{weight_dir}/netG_best_recall.pth')
            torch.save({'epoch': epoch, 'state_dict': self.netd.state_dict()}, f'{weight_dir}/netD_best_recall.pth')
        elif type == 0:
            torch.save({'epoch': epoch, 'state_dict': self.netd.state_dict()}, f"{weight_dir}/netD__recall_{epoch}.pth")
            torch.save({'epoch': epoch, 'state_dict': self.netg.state_dict()}, f"{weight_dir}/netG__recall_{epoch}.pth")


    def load_weights(self, epoch=None, is_best:bool=True, path=None):
        ...
        if is_best:
            fname_g = f"netG__recall_16.pth"
            print("in best part \n")
            fname_d = f"netD__recall_16.pth"
        else:
            print("in else part \n")
        ...
        if path is None:
            print("in 2nd if lse part \n")
            path_g = f"/home/ccg1/Desktop/Krupa_Rushikesh/modi_skip/{fname_g}"
            path_d = f"/home/ccg1/Desktop/Krupa_Rushikesh/modi_skip/{fname_d}"
    ...
    def train(self):
        ...
        for self.epoch in range(self.opt.iter, self.opt.niter):
            ...
            if res2['Recall'] > best_recall:
                best_recall = res2['Recall']
                self.save_weights(self.epoch,0)
            self.visualizer.print_current_performance(res2, best_recall,0)
            
            if res['AUC'] > best_auc:
                best_auc = res['AUC']
                self.save_weights(self.epoch,1)
            self.visualizer.print_current_performance(res2, best_auc,1)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if self.opt.load_weights:
                path = "/home/ccg3/Desktop/Krupa_Rushikesh/modi_skip/netG__recall_16.pth"
            ...
            auc,recall = evaluate(self.gt_labels, self.an_scores, metric=self.opt.metric)
            performance = OrderedDict([('Avg Run Time (ms/batch)', self.times), ('AUC', auc)])
            performance2 = OrderedDict([('Avg Run Time (ms/batch)', self.times),('Recall',recall)])
