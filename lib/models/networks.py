...
import numpy as np
from lib.models.attention_augmented_conv import AugmentedConv
    ...
    def __init__(self, opt):
        ...
        for t in range(n_extra_layers):
            feat.add_module('extra-layers-{0}-{1}-conv'.format(t, cndf),
                            nn.utils.spectral_norm(nn.Conv2d(cndf, cndf, 3, 1, 1, bias=False)))
            #feat.add_module('extra-layers-{0}-{1}-batchnorm'.format(t, cndf),nn.BatchNorm2d(cndf))
        ...
        while csize > 4:
            ...
            feat.add_module('pyramid-{0}-{1}-conv'.format(in_feat, out_feat),
                            nn.utils.spectral_norm(nn.Conv2d(in_feat, out_feat, 4, 2, 1, bias=False)))
            #feat.add_module('pyramid-{0}-batchnorm'.format(out_feat),nn.BatchNorm2d(out_feat))
            feat.add_module('pyramid-{0}-relu'.format(out_feat),
                            nn.LeakyReLU(0.2, inplace=True))
            cndf = cndf * 2
            csize = csize / 2

        #augmented_conv1 = AugmentedConv(in_channels=3, out_channels=20, kernel_size=3, dk=40, dv=4, Nh=4, relative=True, stride=2, shape=32).to(device)

        feat.add_module('extra-layers-{0}-{1}-conv'.format(out_feat, out_feat),
                            AugmentedConv(out_feat, out_feat, 3,dk=40, dv=4, Nh=4))
...
def define_G(opt, norm='batch', use_dropout=False, init_type='normal'):
    ...
    netG = NestedUNet()#UnetGenerator(opt.nc, opt.nc, num_layer, opt.ngf, norm_layer=norm_layer, use_dropout=use_dropout)
...
##UnetGenerator(opt.nc, opt.nc, num_layer, opt.ngf, norm_layer=norm_layer, use_dropout=use_dropout)
...
class VGGBlock(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels):
        ...
        self.conv1 = nn.utils.spectral_norm(nn.Conv2d(in_channels, middle_channels, 3, padding=1))
        #self.sn = nn.utils.spectral_norm(module, name='weight', n_power_iterations=1, eps=1e-12, dim=None)
        
        #self.bn1 = nn.BatchNorm2d(middle_channels)
        self.conv2 = nn.utils.spectral_norm(nn.Conv2d(middle_channels, out_channels, 3, padding=1))
